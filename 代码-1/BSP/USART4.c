...
void UART4_Init(uint32_t __Baud)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART4, ENABLE);//开启串口4的时钟

	USART_DeInit(UART4);//大概意思是解除此串口的其他配置
	...
	USART_Init(UART4,&USART_InitStructure);//将相关参数初始化给串口1
	
	USART_ClearFlag(UART4,USART_FLAG_RXNE);//初始配置时清除接收置位
	USART_ClearFlag(UART4,USART_FLAG_IDLE);//初始配置时清除空闲置位

	USART_ITConfig(UART4, USART_IT_RXNE, ENABLE);//初始配置接收中断
	//USART_ITConfig(UART4, USART_IT_IDLE, ENABLE);//初始配置空闲中断

	USART_Cmd(UART4,ENABLE);//开启串口1
	
	NVIC_InitTypeDef NVIC_InitStructure;//中断控制结构体变量定义

	NVIC_InitStructure.NVIC_IRQChannel                    = 52;//中断通道指定为USART4
...
void TJCPrintf(const char *str, ...)
{
	...
	USART_SendData(UART4, end);			//这个函数改为你的单片机的串口发送单字节函数
	while(USART_GetFlagStatus(UART4, USART_FLAG_TXE) == RESET);	//等待发送完毕
	USART_SendData(UART4, end);			//这个函数改为你的单片机的串口发送单字节函数
	while(USART_GetFlagStatus(UART4, USART_FLAG_TXE) == RESET);	//等待发送完毕
	USART_SendData(UART4, end);			//这个函数改为你的单片机的串口发送单字节函数
...
{
	if(USART_GetITStatus(UART4, USART_IT_RXNE) == SET)//判断是不是真的有中断发生
	{
//		USART_SendData(UART4,USART_ReceiveData(UART4));//又将数据发回去(用于验证)

		writeRingBuff(USART_ReceiveData(UART4));
		USART_ClearITPendingBit(UART4, USART_IT_RXNE); //已经处理就清楚标志位 
	}  
}
